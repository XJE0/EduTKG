...
# 动态伪标签生成
def generate_dynamic_pseudo_labels(sentences, model, tokenizer, device, confidence_threshold=0.9):
...
def main():
    ...
    for cycle in range(max_cycles):
        ...
        # 生成伪标签
        pseudo_sentences, pseudo_labels = generate_dynamic_pseudo_labels(sentences, teacher_model, tokenizer, device, confidence_threshold=0.9)
    ...
    student_model_path = f'model/student_model'
    student_model.save_pretrained(student_model_path)
    tokenizer.save_pretrained(student_model_path)
